...
export function useAppUpdateNotifications() {
  ...
  const checkForAppUpdates = useCallback(async () => {
    ...
    try {
      ...
      // Non-critical: the auto update-check hits GitHub's releases API, which
      // rate-limits unauthenticated requests with a 403. Log at warn so it
      // doesn't surface as a dev error overlay — a failed background check is
      // harmless (the manual check still reports errors normally).
      console.warn("Skipping app update check:", error);
    }
