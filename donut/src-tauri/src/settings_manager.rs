...
pub struct AppSettings {
  ...
  // Marine generation provider config. The OpenAI-compatible API key is NOT
  // stored here (no plaintext secrets) — it is read from the
  // DONUT_MARINE_OPENAI_API_KEY environment variable.
  #[serde(default)]
  ...
  #[serde(default)]
  pub marine_openai_model: Option<String>,
}
...
pub async fn save_app_settings(
  ...
  // Marine requires the local API and force-enables it at startup (see the
  // Marine auto-start block in lib.rs). A frontend dialog that snapshotted its
  // settings before that flip would otherwise send a stale api_enabled=false on
  // any save, which below would delete the token and disable the API out from
  // under the running extension. So preserve a disk-enabled API here: if the
  // API is enabled on disk, keep it enabled regardless of the incoming value.
