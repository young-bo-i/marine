...
// 「本地智能体」通过本地 HTTP API（Marine）生成直评/回复并可填入页面回复草稿。
...
// 按当前页内容选中 1–2 段母稿；旧评论库不再整份注入，避免平均语气。
async function loadSkill(context) {
  if (!skillCache) {
    const base = 'skills/' + SKILL_BRAND + '/';
    const [brand, execution, style, mother, index] = await Promise.all([
      fetchText(base + '品牌.md'),
      fetchText(base + '执行口径.md'),
      fetchText(base + '风格参数.json'),
      fetchText(base + '母稿.md'),
      fetchText(base + '母稿索引.json'),
    ]);
    skillCache = { brand, execution, style, mother, index };
  }
  let customSample = '';
  // 用户导入的本地补充范文（.md，存 chrome.storage）只能作为母稿之后的次级参照。
  try {
    ...
    if (o && o.marineCustomSampleMd && o.marineCustomSampleMd.trim()) {
      customSample = ['## ' + (o.marineCustomSampleName || 'custom.md'), '', o.marineCustomSampleMd.trim()].join('\n');
    }
  } catch (e) {}
  return marineScholayBuildSkill(skillCache, context, customSample);
}
...
$('#btn-codex').addEventListener('click', async () => {
  if (!lastGrab) return;
  if (!configReady()) { setStatus('未连接本地 API —— 请在「配置」中手动连接，或由 Marine 自动注入', 'error'); return; }
  $('#btn-codex').disabled = true;
  renderReplies(null);
  setStatus('本地智能体生成中…（首次可能十几秒）');
  try {
    const payload = marineRimeBuildPayload(lastGrab, {
      platform: lastGrab.platform,
      url: lastGrab.url,
      title: lastGrab.title,
      mode: 'direct',
      targetSummary: '当前作品的直评与评论区回复候选',
    });
    const res = await apiFetch('/generate', {
      method: 'POST',
      body: JSON.stringify({ skill: await loadSkill(payload), payload }),
    });
...
function renderReplies(res) {
  ...
  async function markPosted(opts, text, btn) {
    ...
    try {
      if (!effectiveConfig.profileId) throw new Error('请先到「配置」选择 Marine 身份');
      const confirmedText = window.prompt('请确认实际发布的文字（如在网页中改过，请在这里同步修改）：', text || '');
      if (confirmedText == null) {
        btn.textContent = old; btn.disabled = false;
        return;
      }
      if (!confirmedText.trim()) throw new Error('实际发布文字不能为空');
      const replyTarget = opts.reply ? findReplyTarget(opts.reply) : null;
      await apiFetch('/history', {
        ...
        body: JSON.stringify({
          ...
          page_title: (lastGrab && lastGrab.title) || '',
          platform: (lastGrab && lastGrab.platform) || '',
          kind: opts.kind || 'direct',
          angle: opts.angle || '',
          text: confirmedText,
          target_comment_id: replyTarget && replyTarget.id ? String(replyTarget.id) : '',
          target_author: replyTarget && replyTarget.authorName ? String(replyTarget.authorName) : '',
        }),
      });
      btn.textContent = '已发'; btn.classList.add('done');
      setStatus('已记录到发布历史（手动确认）', 'ok');
    } catch (e) {
      btn.textContent = old; btn.disabled = false;
  ...
  function card(head, text, opts) {
    ...
    if (lastGrab && lastGrab.platform === 'bilibili') {
      const automatic = document.createElement('span');
      automatic.className = 'reply-auto-note';
      automatic.textContent = '发布成功后自动记录';
      actions.appendChild(automatic);
    } else {
      const post = document.createElement('button');
      post.className = 'reply-copy reply-post';
      post.textContent = '手动确认已发';
...
(async function init() {
  ...
  if (configReady()) {
    ...
    setStatus('未连接本地 API —— 请在「配置」中手动连接，或由 Marine 自动注入', '');
  }
})();
