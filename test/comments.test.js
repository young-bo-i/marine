...
const agentMd = ctx.marineCommentsForAgent(built.comments, 100);
assert(/\[id=90000000000000002 10赞 北京\] 楼主A：根评论A/.test(agentMd), '智能体评论上下文包含根评论 id');
assert(/↳ \[id=90000000000000003 1赞\] 子B：回复A的第一条/.test(agentMd), '智能体评论上下文包含楼中楼 id');

const targets = ctx.marineFlattenComments(built.comments);
const targetA = targets.find(t => t.id === '90000000000000002');
assert(targetA && targetA.authorName === '楼主A' && targetA.snippet === '根评论A', '扁平目标索引保留 id/作者/片段');
